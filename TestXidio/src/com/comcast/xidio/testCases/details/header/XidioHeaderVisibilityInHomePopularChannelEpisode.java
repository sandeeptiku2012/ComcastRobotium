...
public class XidioHeaderVisibilityInHomePopularChannelEpisode extends ActivityInstrumentationTestCase2<FirstRun> {

	private Solo solo;
	String elementTitle =null;

	public XidioHeaderVisibilityInHomePopularChannelEpisode() {
	...
	public void testXidioHeaderVisibilityInHomePopularChannelEpisode()
	{
				...
				for (int count = 0; count < popularJsonArray.length(); count++) 
				{
					...
					if(currentElement.getString("contentType").contentEquals("content_category")){
						...
						{	 
							...
							JSONArray epContent = GetEpisodesList.getInstance().getEpisodeList(channelContentKey);
							solo.sleep(TestConstants.SLEEP_TIME_2000);
							if(epContent!=null){
								for(int j=0; j<epContent.length();j++){
