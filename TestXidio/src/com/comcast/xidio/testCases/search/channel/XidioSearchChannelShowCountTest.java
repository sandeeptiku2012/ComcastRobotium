...
public class XidioSearchChannelShowCountTest extends	ActivityInstrumentationTestCase2<FirstRun> {
	private Solo solo;

	public XidioSearchChannelShowCountTest() {
	...
	public void testXidioSearchChannelShowCount() 
	{
...
//		solo.sendKey(KeyEvent.KEYCODE_DPAD_DOWN);
		...
		ArrayList<JSONObject> channelSearchArray =GetChannelSearchList.getInstance().getChannelSearchList("mac");
		solo.sleep(TestConstants.SLEEP_TIME_5000);
		solo.sendKey(KeyEvent.KEYCODE_DPAD_DOWN);
		solo.sendKey(KeyEvent.KEYCODE_DPAD_DOWN);
		solo.sendKey(KeyEvent.KEYCODE_DPAD_DOWN);
			...
			try {
				if(currentObject.get("level").equals("SHOW")){	 
					...
					if(descString.equalsIgnoreCase("0 shows")){
						...
						assertNull(GetShowContent.getInstance().getShowContent(currentObject.getString("@id")));
					}
					else{
						assertNotNull(GetShowContent.getInstance().getShowContent(currentObject.getString("@id")));
					}
				...
				Log.e(this.getClass().getCanonicalName(), "Failed to complete the test XidioSearchChannelShowCount " , e);
			}
		}
