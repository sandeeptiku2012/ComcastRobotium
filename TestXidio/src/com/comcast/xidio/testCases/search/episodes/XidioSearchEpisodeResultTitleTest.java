...
public class XidioSearchEpisodeResultTitleTest extends ActivityInstrumentationTestCase2<FirstRun> {
	
	private Solo solo;


	public XidioSearchEpisodeResultTitleTest() {
...
//		solo.sendKey(KeyEvent.KEYCODE_DPAD_DOWN);
		...
		for (JSONObject currentObject : channelSearchArray) {
			try {
				if(currentObject.get("level").equals("SHOW") || currentObject.get("level").equals("SUB_SHOW") ){
					assertTrue(false);
				}
				else{
