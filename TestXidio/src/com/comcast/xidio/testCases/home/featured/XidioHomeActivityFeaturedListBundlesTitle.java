...
public class XidioHomeActivityFeaturedListBundlesTitle extends ActivityInstrumentationTestCase2<FirstRun> {

	private Solo solo;
	String elementTitle =null;

	public XidioHomeActivityFeaturedListBundlesTitle() {
	...
	public void testXidioHomeActivityFeaturedListBundlesTitle()
	{
...
//		solo.sendKey(KeyEvent.KEYCODE_DPAD_DOWN);
		...
		try {
			...
			if(featuredJsonArray!=null && featuredJsonArray.length()>0)
			{
				...
				for (int count = 0; count < featuredJsonArray.length(); count++) 
				{
					...
					if(currentElement.has("contentType"))
					{
		...
		{
			Log.e(this.getClass().getCanonicalName(), "Failed to complete the test XidioHomeActivityFeaturedListBundlesTitle " , e);
			assertTrue(false);
		}
