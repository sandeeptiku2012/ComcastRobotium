...
public class XidioSubscriptionEpisodeActivity extends
		ActivityInstrumentationTestCase2<FirstRun> {
	private Solo solo;
	private JSONObject currChannel;

	public XidioSubscriptionEpisodeActivity() {
	...
	public void testXidioSubscriptionEpisodeActivity() {
		...
		JSONArray subscribedChannels = GetSubscriptionList.getInstance()
				.getSubscriptionList(XidioApplication.getLastLoggedInUser());
		...
		if(subscribedChannels==null || subscribedChannels.length()==0) {
			...
			solo.sleep(TestConstants.SLEEP_TIME_2000);
			solo.sendKey(KeyEvent.KEYCODE_DPAD_UP);
			subscribedChannels = GetSubscriptionList
					.getInstance()
					.getSubscriptionList(XidioApplication.getLastLoggedInUser());
		...
		for (int i = 0; i < subscribedChannels.length(); i++) {
			...
			try {
				...
				JSONArray subsShowContent = GetShowContent.getInstance()
						.getShowContent(subsContentKey);
				if (subsContentKey != null) {
					...
					JSONArray episodeListArray = GetEpisodesList.getInstance()
					...
					for (int k = 0; k < episodeListArray.length(); k++) {
						...
						String episodeTitle = currentEpisode
								.getString(TestConstants.TITLE);
						assertTrue(solo.searchText(episodeTitle));
						solo.sendKey(KeyEvent.KEYCODE_DPAD_CENTER);
						assertTrue(solo
								.waitForActivity(TestConstants.VIDEOPLAYER_ACTIVITY));
						solo.sleep(TestConstants.SLEEP_TIME_1000);
						solo.sendKey(KeyEvent.KEYCODE_BACK);
						solo.sleep(TestConstants.SLEEP_TIME_500);
						solo.sendKey(KeyEvent.KEYCODE_DPAD_RIGHT);
				...
				Log.e("Exception:",
						"Exception occured in XidioSubscriptionEpisodeActivity test",
						e);
			}
		}
		solo.sendKey(KeyEvent.KEYCODE_BACK);
